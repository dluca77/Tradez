...
def _active_risk_gate(controller: AutonomousTradingController, equity: float) -> dict | None:
    # Shared with the Discord bot's /status command (tradingbot/risk_status.py)
    # so the two surfaces can never disagree about whether a gate is active.
    return active_risk_gate(controller.state, controller.cfg.risk, equity)
